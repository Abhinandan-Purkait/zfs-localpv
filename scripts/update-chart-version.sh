...
check_tag_is_valid() {
    ...
    local major_minor_current=$(get_major_minor "$current_chart_version")

    if [[ "$current_chart_version" == *"-prerelease" ]]; then
        local major_minor_tag=$(get_major_minor "$tag")
        if [[ "$major_minor_current" != "$major_minor_tag" ]]; then
            die "For prerelease, X.Y from current chart version ($major_minor_current) must exactly match X.Y from tag ($tag)"
        fi
    elif [[ "$current_chart_version" == *"-develop" ]]; then
        local major_minor_tag=$(get_major_minor "$(semver bump minor $tag)")
        if [[ "$major_minor_current" != "$major_minor_tag" ]]; then
            die "For develop, X.Y-1 from current chart version ($major_minor_current) must exactly match X.Y from tag ($tag)"
        fi
    fi
}
...
create_version_from_tag() {
  if [[ "$TAG" =~ ^(v[0-9]+\.[0-9]+\.[0-9]+)$ ]]; then
    ...
    check_tag_is_valid "$CURRENT_CHART_VERSION" "$EXTRACTED_VERSION"
